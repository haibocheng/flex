 ...
 */
	...
	public class FlowElement implements ITextLayoutFormat
	{
		...
		*/
		...
		/** @private
		 * True if the FlowElement can be automatically removed by merging with adjacent elements, or becoming empty.
		 */
		tlf_internal function get canMerge():Boolean
		{
			return true;
		}
		
		/** @private */
		tlf_internal function set canMerge(value:Boolean):void
		{
		}
		...
		* @playerversion AIR 1.5
		...
		{ return _formatValueHolder ? _formatValueHolder.getPrivateData(idString) : null; }
		public function set id(val:String):void
		{
			if (val != id)
			{
				writableTextLayoutFormatValueHolder().setPrivateData(idString,val);
		...
		{ return this._formatValueHolder ? _formatValueHolder.getPrivateData(styleNameString) : null; }
		public function set styleName(val:String):void
		{
			if (val != styleName)
			{
				writableTextLayoutFormatValueHolder().setPrivateData(styleNameString,val);
				modelChanged(ModelChange.STYLE_SELECTOR_CHANGED,0,this.textLength);
			}
		}
