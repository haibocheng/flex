...
package flashx.textLayout.conversion 
{
	...
	{	
		...
		static private function copyAllStyleProps(dst:FlowLeafElement,src:FlowLeafElement):void
		{
			...
			for each (var child:XML in elemList) 
			{
				...
				else if (elemName == "br")
				{
					...
					if (brElem)
					{
						...
						importFilter.reportError("Unexpected element " + elemName + "within a span");
				}
		...
		public function parseAttributes(xmlToParse:XML,formatImporters:Array):void
		{
			...
			for each (var item:XML in xmlToParse.attributes())
			{
				...
				if (!imported)	// not a supported attribute
					reportError("Attribute " + propertyName + " not permitted in element " + xmlToParse.name().localName); //  + " not allowed in element " + xmlToParse.localName());
			}
		}
		...
		public function parseFlowGroupElementChildren(xmlToParse:XML, parent:FlowGroupElement, exceptionElements:Object = null, chainedParent:Boolean=false):void
		{
			for each (var child:XML in xmlToParse.children())
			{
				...
				{
					var info:FlowElementInfo = getElementInfo(child);
					
					if (!info)
					{
						if (exceptionElements == null || exceptionElements[child.name().localName] === undefined)
							reportError("Unknown element " + child.name());
						continue;
					}
					parseObject(child.name().localName, child, parent);
				}
		...
		{
			reportError("missing override for createParagraphFlowFromXML");	// really a coding error - client must override
			return null;
		}
		
		protected function parseObject(name:String, xmlToParse:XML, parent:FlowGroupElement):void
		{
			...
			if (info != null)
				...
				reportError("Unexpected Element of Type " + xmlToParse.localName() + " found.");
		}
