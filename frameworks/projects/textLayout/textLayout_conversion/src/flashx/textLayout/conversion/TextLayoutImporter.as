	...
	public class TextLayoutImporter extends BaseTextLayoutImporter 
	{
		...
		* @playerversion AIR 1.5
		...
		override protected function parseContent(rootStory:XML):TextFlow
		{
			...
			if (!textFlowElement)
			{
				reportError("No TextFlow to parse");
				return null;
			}
		...
		/** @private */
		static private const textFlowExceptionElements:Object = { format:1 };		
		...
		override public function createTextFlowFromXML(xmlToParse:XML, textFlow:TextFlow = null):TextFlow
		{
			...
			parseFlowGroupElementChildren(xmlToParse, newFlow, textFlowExceptionElements);
		...
		{				
			...
			if (_ilgFormatImporter.result)
			{
				...
				imgElem.width  = InlineGraphicElement.widthPropertyDefinition.valueFromString(_ilgFormatImporter.result["width"]);
				/*	We don't support rotation yet because of bugs in the player. */		
				// imgElem.rotation  = InlineGraphicElement.heightPropertyDefinition.valueFromString(_ilgFormatImporter.result["rotation"]);
				imgElem.float = String(InlineGraphicElement.floatPropertyDefinition.valueFromString(_ilgFormatImporter.result["float"]));
			}
