...
{	
	...
	import flashx.textLayout.acc.TextAccImpl;
	...
	public class StandardFlowComposer extends FlowComposerBase implements IFlowComposer
	{
		...
		*/
		...
		public function findControllerIndexAtPosition(absolutePosition:int,preferPrevious:Boolean=false):int
		{
			...
			for (var idx:int = 0; idx < _controllerList.length; idx++)
			{
				var cont:ContainerController = _controllerList[idx];
				if (absolutePosition >= cont.absoluteStart)
				{
					var controllerEnd:int = cont.absoluteStart+cont.textLength;
					if (absolutePosition < controllerEnd || preferPrevious && absolutePosition == controllerEnd)
						return idx;
				}
			}
			return -1;	
		...
		public function compose():Boolean
		{
			...
			return _composing ? false : internalCompose();
		}
		...
		public function composeToPosition(absolutePosition:int = int.MAX_VALUE):Boolean
		{
			...
			return _composing ? false : internalCompose(absolutePosition, -1);
		}
		...
		public function composeToController(index:int = int.MAX_VALUE):Boolean
		{
			...
			return _composing ? false : internalCompose(-1, index);
		}
