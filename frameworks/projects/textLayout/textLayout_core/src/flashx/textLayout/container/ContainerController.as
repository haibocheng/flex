...
package flashx.textLayout.container 
{
	 ...
	 * A TextFlow has an IFlowComposer, which manages a container or a set of containers for the TextFlow. The
	 * IFlowComposer has an ContainerController for each container it manages. The ContainerController 
	 * manages the container the attributes that are unique to that container and the text
	 * in the container.
	 *
	...
	{		
		...
		*/
		...
		tlf_internal function setTextLength(numChars:int):void
		{
			...
			if (textFlow)
			{
				...
				if (numChars != 0 && flowComposer.getControllerIndex(ContainerController(this)) == flowComposer.numControllers - 1 &&
		...
		protected function fillShapeChildren(sc:Array):void
		{ 
			if (_textLength == 0)
				return;	// none				
			...
			for (var lineIndex:int = firstLine; lineIndex <= lastLine; lineIndex++)
			{
				...
				var curBounds:Rectangle = ContainerController(this).getPlacedTextLineBounds(textLine); 
			...
			if (curEstimatedWidth > _compositionWidth && !_measureWidth)
			{
				...
				if (wmode == BlockProgression.RL)
				{
					...
					if (okToCompose && _composeCompleteRatio != 1 && newScroll != _xScroll)
					{
						...
						flowComposer.composeToController(flowComposer.getControllerIndex(ContainerController(this)));
		...
		{
			var flowComposer:IFlowComposer = textFlow.flowComposer;
			flowComposer.updateToController(flowComposer.getControllerIndex(ContainerController(this)));
		...
		{
			if (_rootElement)
				_columnState.updateInputs(effectiveBlockProgression, _rootElement.computedFormat.direction, ContainerController(this), _compositionWidth, _compositionHeight);
		}
		...
		tlf_internal function attachTransparentBackgroundForHit(justClear:Boolean):void
		{
			if (_minListenersAttached && attachTransparentBackground)
			{
				...
				if (s)
				{
					...
					var height:Number = _measureHeight ? _contentHeight : _compositionHeight;
					s.graphics.clear();
					if (!justClear && width && height)
					{
						var wmode:String = effectiveBlockProgression;			
						s.graphics.beginFill(0, 0);
						//Fix for Watson 2347938 - if the scrolls are off, and we're in TTB text, then we should not
						//offset the background.  Using a var because this code was hard to read and is clearer in this form, 
						//even if slightly inefficient.
						var adjustVerticalScroll:Boolean = wmode == BlockProgression.RL && _horizontalScrollPolicy != ScrollPolicy.OFF && _verticalScrollPolicy != ScrollPolicy.OFF;
						s.graphics.drawRect(adjustVerticalScroll ? _xScroll - width : _xScroll, _yScroll, width, height);
						s.graphics.endFill();
					}
				}
			} 
		...
		* @playerversion AIR 1.5
		...
		public function getScrollDelta(numLines:int):Number
		{
			...
			if (numLines < 0) 
			{
				...
				if (firstVisibleLine)
				{
					var firstVisibleTextLine:TextLine = firstVisibleLine.getTextLine();
					if (effectiveBlockProgression == BlockProgression.TB)
					{
		...
		*/
		...
		public function menuSelectHandler(event:ContextMenuEvent):void
		{
    		...
    		{
				tf.contextMenu.clipboardItems.copy = false;
				tf.contextMenu.clipboardItems.cut = false;
				tf.contextMenu.clipboardItems.paste = false;
				tf.contextMenu.clipboardItems.selectAll = false;
			}
		}
		...
		tlf_internal function drawPointSelection(selFormat:SelectionFormat, x:Number,y:Number,w:Number,h:Number):void
        {
			...
			ContainerController(this).addSelectionChild(selObj);
        }
		...
		*/
		...
		tlf_internal function updateCompositionShapes():void
		{
			...
			var newShapeChildren:Array = new Array;
			fillShapeChildren(newShapeChildren);
		...
		*/
		...
		tlf_internal function getPlacedTextLineBounds(textLine:TextLine):Rectangle
		{
			...
			if (!textLine.parent)
			{
				...
				_container.addChild(textLine);
				curBounds = textLine.getBounds(DisplayObject(_container));
				_container.removeChild(textLine);
			}
			else
			{
				// Note: Relative to its parent, which may not be _container
				// but in all reasonable cases, should share its origin with _container
				curBounds = textLine.getBounds(DisplayObject(textLine.parent));
			}
