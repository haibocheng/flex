	...
	public class FlowGroupElement extends FlowElement
	{
		...
		* @playerversion AIR 1.5
		...
		public function findChildIndexAtPosition(relativePosition:int):int
		{
			...
			{
				var mid:int = (lo+hi)/2;
				var p:FlowElement = getChildAt(mid);
				if ((relativePosition >= p.parentRelativeStart && relativePosition < p.parentRelativeEnd) || (p.textLength == 0 && p.parentRelativeStart == relativePosition))
				{
					matchIndex = mid;
					hi = mid-1; // continue looking
				}
				else if (p.parentRelativeStart <= relativePosition)
					lo = mid+1;
				else
					hi = mid-1;
			}
			CONFIG::debug { assert((matchIndex != -1) || (relativePosition < 0 || relativePosition >= textLength),"pos in range but no leaf found in FlowGroupElement.findChildAtPosition"); }
			return matchIndex;
		}
		...
		tlf_internal function addChildAfterInternal(child:FlowElement, newChild:FlowElement):void
		{
			...
			{
				if (_singleChild != child)
					throw(new RangeError("bad call to addChildAfter"));	// TODO: what should this do?
		 ...
		 *	 
	         ...
	         * @throws ArgumentError One or more new elements is still owned by another FlowGroupElement.
	         * 
		...
		public function replaceChildren(beginChildIndex:int, endChildIndex:int, ...rest):void
		{
			if (beginChildIndex > _numChildren || endChildIndex > _numChildren)
				throw RangeError("invalid parameter to FlowGroupElement.replaceChildren");	
				...
				for (idx; idx<numNestedArgs; idx++)
				{
					...
					if (newChild)
					{
						...
						if (!canOwnFlowElement(newChild))
							throw ArgumentError("newElement not of a type that this can be parent of");
		...
		public function removeChild(child:FlowElement):FlowElement
		{
			...
			if (index == -1)
				throw ArgumentError("Child to remove not found.");
