...
package flashx.textLayout.elements 
{
	...
	{	
		...
		public function InlineGraphicElement():void
		{
			...
			_measuredWidth = 0;
			_measuredHeight = 0;
			internalSetWidth(null);
			internalSetHeight(null);
			_graphicStatus = InlineGraphicElementStatus.LOAD_PENDING;
			_float = floatPropertyDefinition.defaultValue as String;
			setTextLength(1);
		}
	...
	//		trace("text is ", _blockElement.rawText.charCodeAt(0), "length", _blockElement.rawText.length);
		...
		* @playerversion AIR 1.5
		...
		/** The width of the graphic. The value can be 'auto', a number of pixels or a percent of the measured 
		* width of the image.
		*
		* @playerversion Flash 10
		* @playerversion AIR 1.5
		...
		{ return _width; }
		public function set width(w:Object):void
		{ 
			internalSetWidth(w);
			modelChanged(ModelChange.ELEMENT_MODIFIED,0,textLength);
		}
		...
		* @playerversion AIR 1.5
		...
		{ return _width is String; }
		
		private function computeWidth():Number
		{
			CONFIG::debug { assert(widthIsComputed(),"bad call to InlineGraphicElement.computeWidth"); }
			if (_width == FormatValue.AUTO)
			{
				if (_height == FormatValue.AUTO)
					return _measuredWidth;
				if (_measuredHeight == 0 || _measuredWidth == 0)
					return 0;
				// can't rely on height being calculated yet
				var effHeight:Number = heightIsComputed() ? computeHeight(): Number(_height);
				return effHeight/_measuredHeight * _measuredWidth;
			}
			return widthPropertyDefinition.computeActualPropertyValue(_width,_measuredWidth); 
		}
		
		private function internalSetWidth(w:Object):void
		{
			if (w == null)
				w = widthPropertyDefinition.defaultValue;
			_width = widthPropertyDefinition.setHelper(width,w);
			elementWidth = widthIsComputed() ? 0 : Number(_width);
		...
		* @playerversion AIR 1.5
		...
		{ return _height; }
		public function set height(h:Object):void
		{
			internalSetHeight(h);
			modelChanged(ModelChange.ELEMENT_MODIFIED,0,textLength);
		}
		...
		tlf_internal function get float():String
		{
			return _float;
		}
		/** @private - prototype phase only */
		tlf_internal function set float(value:String):void
		{
		...
		* @playerversion AIR 1.5
		...
		private function computeHeight():Number
		{
			...
			if (_height == FormatValue.AUTO)
			{
				...
				var effWidth:Number = widthIsComputed() ? computeWidth(): Number(_width);
				return effWidth/_measuredWidth * _measuredHeight;
			}
			return heightPropertyDefinition.computeActualPropertyValue(_height,_measuredHeight); 
		}
		
		private function internalSetHeight(h:Object):void
		{
			if (h == null)
				h = heightPropertyDefinition.defaultValue;
			_height = heightPropertyDefinition.setHelper(height,h);
			elementHeight = heightIsComputed() ? 0 : Number(_height);
		...
		{			
			CONFIG::debug { Debugging.traceFTECall(null,null,"loadCompleteHandler()"); }
		...
		tlf_internal override function applyDelayedElementUpdate(textFlow:TextFlow,okToUnloadGraphics:Boolean,hasController:Boolean):void
		{
			...
			if (_graphicStatus == InlineGraphicElementStatus.LOAD_PENDING)
			{
				...
				if (hasController)
				{
					...
					if (source is String)
					{
						var inlineGraphicResolver:Function = textFlow.configuration.inlineGraphicResolver;
		...
		/** @private */
		tlf_internal override function calculateStrikeThrough(tLine:TextLine, blockProgression:String, metrics:FontMetrics, spanBounds:Rectangle):Number
		{
			if (!this.graphic || status != InlineGraphicElementStatus.READY)
				return super.calculateStrikeThrough(tLine,blockProgression,metrics,spanBounds);
