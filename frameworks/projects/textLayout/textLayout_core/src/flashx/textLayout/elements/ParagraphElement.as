...
package flashx.textLayout.elements 
{
	...
	public final class ParagraphElement extends ParagraphFormattedElement
	{
		...
		* @playerversion AIR 1.5
		...
		static tlf_internal const textBlockHasReleaseLineCreationData:Boolean = new TextBlock().hasOwnProperty("releaseLineCreationData");
		/** @private */
				...
				for (;;)
				{
					...
					if (idx == numChildren)
					{
						...
						if (idx != 0)
						{
							...
							if (lastChild is SubParagraphGroupElement && lastChild.textLength == 1)
		...
		tlf_internal static function getLeadingBasis (leadingModel:String):String
		{
			switch (leadingModel)
			{
				...
				default:
					throw Error("Unsupported parameter to ParagraphElement.getLeadingBasis"); // In particular, AUTO is not supported by this method. Must be mapped to one of the above 
			}
		}
		...
		tlf_internal static function useUpLeadingDirection (leadingModel:String):Boolean
		{
			switch (leadingModel)
			{
				...
				case LeadingModel.IDEOGRAPHIC_CENTER_DOWN:
					return false;
				default:
					throw Error("Unsupported parameter to ParagraphElement.useUpLeadingDirection"); // In particular, AUTO is not supported by this method. Must be mapped to one of the above 
			}
		}
	}
}
