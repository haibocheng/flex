...
*/
	public final class StringTextLineFactory extends TextLineFactoryBase
	{
		...
		private function createTextLinesInternal(callback:Function):void
		{
			...
			// - compose width is specified (for TB blockProgression) or compose height is specified for (RL blockProgression)
			// - explicit line breaking is not used
			// - content doesn't fit		
			if (truncationOptions && (bp == BlockProgression.TB ? !measureWidth : !measureHeight) && _tf.computedFormat.lineBreak != LineBreak.EXPLICIT && !doesComposedTextFit(truncationOptions.lineCountLimit, _tf.textLength, bp))
			{
				_isTruncated = true;
				...
				if (_truncationLineIndex >= 0)
				{
					...
					if (_truncationLineIndex >= 0)
					{
						...
						var truncateAtCharPosition:int = getTruncationPosition(_factoryComposer._lines[_truncationLineIndex], allowedWidth);
