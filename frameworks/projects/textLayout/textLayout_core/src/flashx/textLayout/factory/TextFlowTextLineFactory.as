...
*/
	public final class TextFlowTextLineFactory extends TextLineFactoryBase
	{
		...
		public function createTextLines(callback:Function,textFlow:TextFlow):void
		{
			...
			var measureHeight:Boolean = isNaN(compositionBounds.height);
			...
			// - content doesn't fit		
			if (truncationOptions && (bp == BlockProgression.TB ? !measureWidth : !measureHeight) && textFlow.computedFormat.lineBreak != LineBreak.EXPLICIT && !doesComposedTextFit(truncationOptions.lineCountLimit, textFlow.textLength, bp))
			{
				_isTruncated = true;
