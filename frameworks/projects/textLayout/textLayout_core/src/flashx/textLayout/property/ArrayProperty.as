...
package flashx.textLayout.property
{
	...
	public class ArrayProperty extends Property
	{
		...
		{ 
			CONFIG::debug { assert(newVal !== null,"Property.setHelper - null passed in did you mean undefined?"); }
			if (newVal == undefined || newVal == FormatValue.INHERIT)
				return newVal;
			if (!checkArrayTypes(newVal))
				newVal = null;
			return newVal == null || !(newVal is Array) ? currVal : (newVal as Array).slice(); 
		}
			...
			for each (var attrs:String in attrsAll)
			{
			 	...
			 	for each (var attr:String in attrsOne)
			 	{
				 	...
				 	for each (var prop:Property in desc)
				 	{
				 		...
				 		{
				 			obj[propName] = prop.valueFromString(propVal);
				 			break;
				 		}
			 		}
			 	}
