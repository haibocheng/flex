...
package flashx.textLayout.property
{
	...
	public class NumberOrPercentProperty extends Property
	{
		...
		static private function toNumberIfPercent(o:Object):Number
		{
			...
			return len != 0 && s.charAt(len-1) == "%" ? Number(s.slice(0,len-1)) : NaN;
		}
		...
		{ 
			...
			if (!isNaN(newVal))
			{
				...
				if (checkLowerLimit() && newVal < _minPercentValue)
					newVal =  _minPercentValue;
				else if (checkUpperLimit() && newVal > _maxPercentValue)
					newVal = _maxPercentValue;
		...
		public override function valueFromString(str:String):*
		{
			...
			if (str == FormatValue.INHERIT)
				return str;				
			var newVal:Number = toNumberIfPercent(str);
			if (!isNaN(newVal))
				return str;
