...
package flashx.textLayout.container
{
	...
	*/
	...
	public class TextContainerManager extends EventDispatcher implements ISWFContext, IInteractionEventHandler, ISandboxSupport
	{
		...
		/**
		 * Returns the current text using separator between paragraphs
		 *
		 ...
		 *
		 * Discard any attached TextFlow. Any selection is lost.
		 * 
		...
		* @playerversion AIR 1.5
		 ...
		 * @param previousLine flash.text.engine:TextLine (default = null)  Specifies the previously broken line after 
		 *	which breaking is to commence. Can be null when breaking the first line.  
		 * @param width Number (default = 1000000). Specifies the desired width of the line in pixels. The 
		 * 	actual width may be less.  
		 * @param lineOffset:Number (default = 0.0)  An optional parameter which specifies the difference in 
		 *	pixels between the origin of the line and the origin of the tab stops. This can be used when lines are not aligned, 
		 * 	but it is desirable for their tabs to be so. The default value for this parameter is 0.0.  
		 * @param  fitSomething:Boolean (default = false) An optional parameter that instructs the runtime to fit at least one 
		...
		*/				
		...
		{
			if (_composeState == COMPOSE_FACTORY)
				_container.contextMenu.clipboardItems.selectAll = _editingMode != EditingMode.READ_ONLY;
			else
