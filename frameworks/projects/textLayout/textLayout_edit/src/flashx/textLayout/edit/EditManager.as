...
package flashx.textLayout.edit
{
	...
	public class EditManager extends SelectionManager implements IEditManager, IIMEClient
	{
		...
		*/		
		 ...
		 * and redoing changes, pass an IUndoManager instance to the EditManager constructor. </p>
		 * 
		 * @param undo	The UndoManager for the application
		 * 
		 * @see flashx.elements.TextFlow#interactionManager
		 * 
		...
		*/
		override public function imeStartCompositionHandler(event:IMEEvent):void
		{
			...
			// level up.
		private var captureLevel:int = 0;
		...
		private function finalizeDo(op:FlowOperation):void
		{
			...
			if (parentStack && parentStack.length > 0)
				parentOperation = parentStack[parentStack.length - 1] as CompositeOperation;
	...
	//		CONFIG::debug { assert(captureLevel == 0 || parentOperation != null, "missing parent for nested operation"); }
		...
		private function doInternal(op:FlowOperation):FlowOperation
		{
			...
			// If we fired off any subsidiary operations, create a composite operation to hold them all
		 	...
		 	{
				op = new CompositeOperation(captureOperations.slice(0, captureOperations.length - captureStart));
				op.setGenerations(FlowOperation(captureOperations[0]).beginGeneration,textFlow.generation);
		...
		public function performUndo(theop:IOperation):void
		{
			...
			try
			{
				...
				if (hasSelection())
					textFlow.flowComposer.getControllerAt(textFlow.flowComposer.numControllers-1).scrollToRange(activePosition,anchorPosition);											
			}
			catch(e:Error)
			{
				opError = e;
		...
		public function deleteNextCharacter(operationState:SelectionState = null):void
		{
			...
			try
			{
				...
				else deleteOp = new DeleteTextOperation(operationState, operationState, false /* don't allow merge when deleting by range */);			
				doOperation(deleteOp);			
			}	
			catch (e:Error)
			{
				if (operationState.absoluteStart == operationState.absoluteEnd && (e is IllegalOperationError))
					throw new Error("Can't delete from a caret selection when text is invalid. Try passing a range selection.");
				else throw e;
			}
		}
		...
		public function applyFormat(leafFormat:ITextLayoutFormat, paragraphFormat:ITextLayoutFormat, containerFormat:ITextLayoutFormat, operationState:SelectionState = null):void
		{
			if (!operationState)
			{
				...
				// remember new Character category formats for next char typed
