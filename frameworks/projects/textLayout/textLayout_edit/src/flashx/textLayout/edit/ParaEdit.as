...
package flashx.textLayout.edit
{
 	...
 	public class ParaEdit
	{
		 ...
		 */
		static private function applyCharacterFormat(leaf:FlowLeafElement, format:ITextLayoutFormat, begIdx:int, rangeLength:int):int
		{
		...
		static private function setCharacterFormat(leaf:FlowLeafElement, format:Object, begIdx:int, rangeLength:int):int
		{
			...
			{				
				...
				// clip rangeLength to the length of this span
		...
		public static function applyTextStyleChange(flowRoot:TextFlow,begChange:int,endChange:int,format:ITextLayoutFormat):void
		{	
			...
			while (workIdx < endChange)
			{
				...
				workIdx = applyCharacterFormat(elem,format,workIdx,endChange-workIdx); 
			}
		}
		...
		public static function splitParagraph(para:ParagraphElement, paraSplitPos:int, pointFormat:ITextLayoutFormat=null):ParagraphElement
		{
			...
			if ((paraSplitPos == para.textLength - 1))
			{
				...
				if (newPar.textLength == 1)
				{
					...
					if (pointFormat != null)
					{
						ParaEdit.setTextStyleChange(para.getTextFlow(),absSplitPos + 1, absSplitPos + 2, pointFormat)
					} else {
			...
			if (para.numChildren == 0)
				para.replaceChildren(0, 0, new SpanElement());
		 ...
		 * @param endIndex		text index within the last paragraph in the range
		 */
		public static function applyParagraphStyleChange(flowRoot:TextFlow,begChange:int,endChange:int,format:ITextLayoutFormat):void
		{	
			...
			while (curIndex <= endChange)
			{
				...
				var paraAttr:TextLayoutFormat = new TextLayoutFormat(para.format);
				paraAttr.apply(format);
				para.format = paraAttr;
		...
		public static function applyContainerStyleChange(flowRoot:TextFlow,begIdx:int,endIdx:int,format:ITextLayoutFormat):void
		{
			CONFIG::debug { assert(begIdx <= endIdx,"bad indexeds passed to ParaEdit.cacheContainerStyleInformation");  }
			if (flowRoot.flowComposer)
			{
				...
				if (controllerIndex >= 0)
				{
					while (controllerIndex < flowRoot.flowComposer.numControllers)
					{
						...
						if (controller.absoluteStart >= endIdx)
							break;
						var containerAttrs:TextLayoutFormat = new TextLayoutFormat(controller.format);
						containerAttrs.apply(format);
						controller.format = containerAttrs;
						controllerIndex++;
					}
				}
			}
