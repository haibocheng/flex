 ...
 * batch1() when -conservative is true or batch2(), the default.
 *
...
//            }
        ...
        try
        {
            ...
            if (preLink != null)  // don't wedge the compiler forever.
            {
                ...
                while (runPrelink && count++ < 1000)
                {
                    ...
                    // SDK-17411 - run preLink only once for compc.
                                               ...
                                               boolean recompile, Configuration configuration)
    {
        return validateCompilationUnits(fileSpec, sourceList, sourcePath, bundlePath, resources, swcContext,
                                        null, perCompileData, recompile, configuration);
    }
    ...
    public static int validateCompilationUnits(FileSpec fileSpec, SourceList sourceList, SourcePath sourcePath,
                                               ...
                                               ContextStatics perCompileData, boolean recompile,
                                               Configuration configuration)
    {
        ...
        for (Source source : resources.sources())
        {
            if (source.getName() == null)
            {
                continue;
        ...
        for (Source source : sources)
        {
            ...
            for (Name name : u.inheritance)
            {
                if (name instanceof QName)
                {
                    Map<String, Source> inheritanceDependents = dependents.get(name);
            ...
            for (Name name : u.namespaces)
            {
                if (name instanceof QName)
                {
                    Map<String, Source> namespacesDependents = dependents.get(name);
            ...
            for (Name name : u.expressions)
            {
                if (name instanceof QName)
                {
                    Map<String, Source> expressionsDependents = dependents.get(name);
            ...
            for (Name name : u.types)
            {
                if (name instanceof QName)
                {
                    Map<String, Source> typesDependents = dependents.get(name);
        ...
        if (recompile || (fileSpec != null && fileSpec.isUpdated()))
        {
            for (Source source : sources)
            {
                ...
                {
                    affected.put(source.getName(), source);
                    if (recompile)
                    {
                        reasons.put(source.getName(), l10n.getLocalizedTextString(new ForceRecompilation()));
                    }
                    else
                    {
                        reasons.put(source.getName(), l10n.getLocalizedTextString(new SourceFileChanged()));
                    }
                }
            }
        ...
        for (Iterator<Source> iterator = sources.iterator(); iterator.hasNext();)
        {
            ...
            for (Name dependentName : dependencies)
            {
                ...
                if (dependentName instanceof QName)
                {
                    qName = (QName) dependentName;
                }
                else if (dependentName instanceof MultiName)
                {
                    MultiName multiName = (MultiName) dependentName;
                    assert multiName.getNumQNames() == 1 : "u = " + u + ", multiName = " + multiName;
                ...
                if (qName != null)
                {
                    ...
                    {
                        String location = dependentSource.getName();
                        ...
                        {
                            if ((perCompileData != null) && (ascContext == null))
                            {
                            ...
                            if (!updatedWithStableSignature.containsKey(location) &&
                                (perCompileData != null) &&
                                u.hasTypeInfo &&
		...
		// must go away too.
		for (Source s : resources.sources())
		{
			if (s != null)
			{
        ...
        if ((count > 0) && (perCompileData != null))
        {
            if (ascContext == null)
            {
                ascContext = new Context(perCompileData);
            ...
            removeVectorTypes(sources, ascContext);
        }
    ...
    /**
     * When ASC encounters a new Vector type, it creates a type on the
     * fly, but Flex has no way to track these types, because there
     * isn't a Source/CompilationUnit for them.  Since there is no way
     * to validate them, we remove references to these types before
     * each incremental compilation.  ASC is smart enough to recreate
     * them as necessary.  See ReferenceValue.bindToTypeParamSlot().
     */
    private static void removeVectorTypes(Set<Source> sources, Context ascContext)
    {
        for (Source source : sources)
        {
            CompilationUnit compilationUnit = source.getCompilationUnit();
            ...
            {
                ObjectValue typeInfo = compilationUnit.typeInfo;
                Names names = typeInfo.builder.getNames();
                ...
                if (names != null)
                {
                    for (int i = 0; (i = names.hasNext(i)) != -1; i++)
                    {
                        ...
                        {
                            String type = name.substring(8, name.length() - 1);

                            if (!ascContext.isBuiltin(type))
                            {
                                ObjectValue namespace = names.getNamespace(i);
                                typeInfo.removeName(ascContext, Tokens.GET_TOKEN, name, namespace);
                                typeInfo.removeName(ascContext, Tokens.SET_TOKEN, name, namespace);
                            }
                        }
                    }
                }
    ...
    static CompilationUnit parse1(Source s, flex2.compiler.SubCompiler[] compilers, SymbolTable symbolTable)
    {
        if (s.isCompiled())
        {
        ...
        if (id.equals("mx.fbpro"))
        {
            if (licenseMap != null)
            {
                ...
                try
                {
                    ...
                    Method method = licenseClass.getMethod("isFlashBuilderPremiumValid", (Class[]) null);
                    result = ((Boolean)method.invoke(instance, (Object[])null)).booleanValue();
                }
                catch (Exception e)
                {
                }
    ...
    public static List<VirtualFile>[] getVirtualFileList(Set fileSet, List<File> directories)
    {
        //TODO this function should really be cleaned up to use Array.newInstance and
        //     List<List<VirtualFile>> instead of an array, so that we can get compile-time type-checks
        @SuppressWarnings("unchecked")
