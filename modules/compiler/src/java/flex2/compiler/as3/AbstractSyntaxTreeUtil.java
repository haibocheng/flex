...
public class AbstractSyntaxTreeUtil
{
                                                                ...
                                                                String expression, int position)
    {
        ...
        if (lastDotIndex > 0)
        {
            if (expression.charAt(lastDotIndex + 1) == '<')
            {
                // Handle Vector.<Foo>
                String vectorString = expression.substring(0, lastDotIndex);
                MemberExpressionNode base = generateGetterSelector(nodeFactory, vectorString, true);
                String elementString = expression.substring(lastDotIndex + 2, expression.length() - 1);
    ...
    public static Node generatePublicVariable(Context context, TypeExpressionNode typeExpression,
    ...
    {
        return parse(context, configNamespaces, text, 0, false);
    }
                                   ...
                                   int lineNumberOffset, boolean emitDocInfo)
    {
        ...
        // Create a new input buffer populated with our code fragment. We enclose
