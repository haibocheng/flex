...
public class BindableSecondPassEvaluator extends GenerativeSecondPassEvaluator
{
                                             ...
                                             boolean addBindableMetaData)
    {
        ...
        if ((fromDefinition != null) && (fromDefinition.metaData != null))
        {
            for (Node node : fromDefinition.metaData.items)
            {
                ...
                if ((metaData.id != null) &&
                    metaData.id.equals(StandardDefs.MD_BINDABLE) &&
                    metaData.count() == 0)
                {
    ...
    private void processBindableMetaData(NodeFactory nodeFactory, MetaDataNode metaData)
    {
        ...
        // Generate the new Bindable metadata body: (event="propertyChange")
        IdentifierNode bindableIdentifier = nodeFactory.identifier(BINDABLE, false);
        IdentifierNode attributeIdentifier = nodeFactory.identifier(EVENT_VAR, false);
        LiteralStringNode literalString = nodeFactory.literalString(PROPERTY_CHANGE);
        ...
        CallExpressionNode callExpression =
            (CallExpressionNode) nodeFactory.callExpression(bindableIdentifier, callExpressionArgumentList);
        callExpression.setRValue(false);
        callExpression.setMode(Tokens.EMPTY_TOKEN);

        // Replace the body of the Bindable metadata.
        MemberExpressionNode memberExpression = (MemberExpressionNode) metaData.data.elementlist.items.get(0);
        memberExpression.selector = callExpression;
