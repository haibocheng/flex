...
public class AsDocAPI
{
			...
			throws ConfigurationException, CompilerException
	{
		...
		List nsComponents = flex2.compiler.swc.SwcAPI.setupNamespaceComponents(configuration.getNamespaces(), mappings,
				sourcePath, classes, configuration.getIncludeLookupOnly(), configuration.isIncludeAllForAsdoc());
		flex2.compiler.swc.SwcAPI.setupClasses(configuration.getClasses(), sourcePath, classes);
		...
		flex2.compiler.CompilerAPI.validateCompilationUnits(fileSpec, sourceList, sourcePath, bundlePath, resources, swcContext,
		...
		CompilerAPI.compile(fileSpec, sourceList, classes.values(), sourcePath, resources, bundlePath, swcContext,
                            mappings, configuration, compilers, new CompcPreLink(null, null), licenseMap, new ArrayList<Source>());
