...
public class ImplementationGenerator extends AbstractGenerator
{
    ...
    private static final String STYLES = "styles".intern();
	...
	{
		return addDescriptorInitializerFragments(nodeFactory, model, includePropNames, false, includeDesignLayer);
	}
                                                                          ...
                                                                          boolean propsOnly, boolean includeDesignLayer)
	{
		...
		argumentList = addDescriptorProperties(nodeFactory, argumentList, model, includePropNames, includeDesignLayer);
     ...
     *
     */
    private static ArgumentListNode addDescriptorProperties(NodeFactory nodeFactory, ArgumentListNode argumentList,
        ...
        if (propIter.hasNext() || vcIter.hasNext() || hasDesignLayer)
        {
            ...
            while (propIter.hasNext())
            {
                ...
                if (!init.isStateSpecific())
                {
                    ...
                    Node valueExpr = init.generateValueExpr(nodeFactory);
            ...
            if (vcIter.hasNext())
            {
                ...
                while (vcIter.hasNext())
                {
                    ...
                    {
                        MemberExpressionNode memberExpression =
                            addDescriptorInitializerFragments(nodeFactory, (MovieClip) init.getValue(), null, true);
    ...
    { 	 
        ...
        { 	 
            ...
            { 	 
                ...
                List<Node> nodeList = AbstractSyntaxTreeUtil.parse(context, configNamespaces, text, 	 
    ...
    private ClassDefinitionNode generateClassDefinition()
    {
        ...
        if (mxmlDocument.getIsIUIComponent())
        {
                ...
                mxmlDocument.getIsContainer())
            {
                ...
                MemberExpressionNode memberExpression =
                    AbstractSyntaxTreeUtil.generateGetterSelector(nodeFactory, standardDefs.getCorePackage(), UI_COMPONENT_DESCRIPTOR, false);
                TypeExpressionNode typeExpression = nodeFactory.typeExpression(memberExpression, true, false, -1);
                MemberExpressionNode initializer = getDescriptorInitializerFragments(nodeFactory, mxmlDocument.getRoot());
            ...
            if (mxmlDocument.getIsContainer())
            {
            ...
            statementList = generateConstructor(statementList, generateDocComments,
                                                className, constructorStatementList);
    ...
    {
        int kind = Tokens.VAR_TOKEN;
        QualifiedIdentifierNode qualifiedIdentifier = generateStaticMxInternalQualifiedIdentifier(name, false);
        MemberExpressionNode memberExpression =
    ...
    private StatementListNode generateStatesInitializers(StatementListNode statementList)
    {
        ...
        if (mxmlDocument.getVersion() >= 4)
        {
            ...
            result = generator.getStatesASTInitializers(mxmlDocument.getStatefulModel(), context, statementList);
        }
        return result;
    }
    ...
    private StatementListNode generateComponentStyleSettings(StatementListNode statementList)
    {
        ...
        if (iterator.hasNext())
        {
            ...
            while (iterator.hasNext())
            {
                ...
                Node rvalue = namedInitializer.generateValueExpr(nodeFactory);
    ...
    private StatementListNode generateConstructor(StatementListNode statementList, boolean generateDocComments,
                                                  String className, StatementListNode constructorStatementList)
    {
        StatementListNode result = statementList;
        ...
        FunctionDefinitionNode constructor =
            AbstractSyntaxTreeUtil.generateConstructor(context, className, true, constructorStatementList, 
                                                       position);
    ...
    {
        int kind = Tokens.VAR_TOKEN;
        QualifiedIdentifierNode qualifiedIdentifier = generateStaticMxInternalQualifiedIdentifier(EFFECTS, false);
        MemberExpressionNode memberExpression =
            AbstractSyntaxTreeUtil.generateGetterSelector(nodeFactory, ARRAY, false);
        TypeExpressionNode typeExpression = nodeFactory.typeExpression(memberExpression, true, false, -1);
        TypedIdentifierNode typedIdentifier = nodeFactory.typedIdentifier(qualifiedIdentifier, typeExpression);
    ...
    private StatementListNode generateEventInitializers(StatementListNode statementList)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            ThisExpressionNode thisExpression = nodeFactory.thisExpression(-1);
            result = initializer.generateAssignExpr(nodeFactory, result, thisExpression);
        }
    ...
    private void generateIfNullEffectsAndPushes(Iterator<String> effectStyleIterator, StatementListNode statementList)
    {
        //if (!effects)
        MemberExpressionNode memberExpression =
        ...
        while (effectStyleIterator.hasNext())
        {
            //effects.push("${effectStyle}");
            ExpressionStatementNode pushExpressionStatement =
                generatePush(EFFECTS, nodeFactory.literalString(effectStyleIterator.next()));
    ...
    private Node generateIfNullStyleDeclaration(String subject, StyleSelector selector)
    {
        ...
        // style = new CSSStyleDeclaration("$styleDecl.subject", selector);
        IdentifierNode styleIdentifier = nodeFactory.identifier(STYLE, false);
        ...
        //  style = new CSSStyleDeclaration();
        {
            ...
            IdentifierNode cssStyleDeclarationIdentifier =
                AbstractSyntaxTreeUtil.generateResolvedIdentifier(nodeFactory, standardDefs.getStylesPackage(),
                                                                  CSS_STYLE_DECLARATION);
            CallExpressionNode callExpression =
                (CallExpressionNode) nodeFactory.callExpression(cssStyleDeclarationIdentifier, null);
            callExpression.is_new = true;
            callExpression.setRValue(false);
            MemberExpressionNode argumentMemberExpression = nodeFactory.memberExpression(null, callExpression);
            ArgumentListNode argumentList = nodeFactory.argumentList(null, argumentMemberExpression);
    ...
    private Node generateIfNullStyleFactory(Iterator<StyleProperty> iterator)
    {
        ...
        StatementListNode statementList;
        {
            MemberExpressionNode base = 
                AbstractSyntaxTreeUtil.generateGetterSelector(nodeFactory, STYLE, false);
            IdentifierNode factoryIdentifier = nodeFactory.identifier(FACTORY, false);
            FunctionCommonNode functionCommon = generateStyleFactory(iterator);
            ArgumentListNode argumentList = nodeFactory.argumentList(null, functionCommon);
    ...
    private StatementListNode generateInitializerSupportDefs(StatementListNode statementList)
    {
        ...
        while (rootSubInitializerIterator.hasNext())
        {
            ...
            result = initializer.generateDefinitions(context, configNamespaces, result, generateDocComments);
        }
    ...
    private ExpressionStatementNode generateInitProtoChainRoots()
    {
        ...
        MemberExpressionNode getterSelector =
            AbstractSyntaxTreeUtil.generateResolvedGetterSelector(nodeFactory, standardDefs.getCorePackage(), MX_INTERNAL);
        QualifiedIdentifierNode qualifiedIdentifier = nodeFactory.qualifiedIdentifier(getterSelector, INIT_PROTO_CHAIN_ROOTS);
        CallExpressionNode selector = (CallExpressionNode) nodeFactory.callExpression(qualifiedIdentifier, null);
        selector.setRValue(false);
        MemberExpressionNode memberExpression = nodeFactory.memberExpression(base, selector);
        ListNode list = nodeFactory.list(null, memberExpression);
        return nodeFactory.expressionStatement(list);
    ...
    private FunctionCommonNode generateRepeatableDestinationFunction(BindingExpression bindingExpression)
    {
        ParameterNode sourceFunctionReturnValueParameter =
            AbstractSyntaxTreeUtil.generateParameter(nodeFactory, _SOURCE_FUNCTION_RETURN_VALUE,
                                                     bindingExpression.getDestinationTypeName(true), true);
    ...
    private StatementListNode generateSourceFunctionArrayConversion(StatementListNode statementList,
                                                                    ...
                                                                    Node initializer)
    {
        // return ((result is Array) || (result is flash.utils.Proxy) ? result : [result]);
        StatementListNode result = statementList;
        ...
        ListNode arrayBinaryExpressionList = nodeFactory.list(null, arrayBinaryExpression);
        ...
        ListNode proxyBinaryExpressionList = nodeFactory.list(null, proxyBinaryExpression);

        BinaryExpressionNode binaryExpression =
            nodeFactory.binaryExpression(Tokens.LOGICALOR_TOKEN, arrayBinaryExpressionList, proxyBinaryExpressionList);
        ...
        ConditionalExpressionNode conditionalExpression =
            nodeFactory.conditionalExpression(binaryExpression, resultMemberExpression, literalArray);
    ...
    {
        MemberExpressionNode staticGetterSelector =
            AbstractSyntaxTreeUtil.generateGetterSelector(nodeFactory, STATIC, false);
        AttributeListNode attributeList = nodeFactory.attributeList(staticGetterSelector, null);
        MemberExpressionNode mxInternalGetterSelector =
            AbstractSyntaxTreeUtil.generateResolvedGetterSelector(nodeFactory, standardDefs.getCorePackage(), MX_INTERNAL);
        ListNode list = nodeFactory.list(null, mxInternalGetterSelector);
        return nodeFactory.attributeList(list, attributeList);
    }

    private QualifiedIdentifierNode generateStaticMxInternalQualifiedIdentifier(String name, boolean intern)
    {
        if (intern)
        {
            name = name.intern();
    }
        return nodeFactory.qualifiedIdentifier(generateStaticMxInternalAttributeList(), name);
    }
    ...
    private FunctionCommonNode generateStyleFactory(Iterator<StyleProperty> iterator)
    {
        //style.factory = function():void
        FunctionSignatureNode functionSignature = nodeFactory.functionSignature(null, null);
        functionSignature.void_anno = true;
    ...
    private StatementListNode generateStylesInitFunction(StatementListNode statementList)
    {
        ...
        while (styleDefIterator.hasNext())
        {
            ...
            initFunctionStatementList = nodeFactory.statementList(initFunctionStatementList, expressionStatement);
                    ...
                    if (styleDeclaration.getProperties().size() > 0)
                    {
                        Node nullStyleFactoryIfStatement =
                            generateIfNullStyleFactory(styleDeclaration.getProperties().values().iterator());
                        initFunctionStatementList = nodeFactory.statementList(initFunctionStatementList,
                    ...
                    if (styleDeclaration.getEffectStyles().size() > 0)
                    {
                        ...
                        generateIfNullEffectsAndPushes(styleDeclaration.getEffectStyles().iterator(),
                                                       initFunctionStatementList);
                    }
                }
            }
            ...
            {
                    Node nullStyleFactoryIfStatement =
                        generateIfNullStyleFactory(styleDef.getStyles().values().iterator());
                initFunctionStatementList = nodeFactory.statementList(initFunctionStatementList,
                                                                      nullStyleFactoryIfStatement);
            }
            ...
            if (styleDef.getEffectStyles().size() > 0)
            {
                    ...
                    generateIfNullEffectsAndPushes(styleDef.getEffectStyles().iterator(),
                                                   initFunctionStatementList);
            }
        }
        }
    ...
    {
        AttributeListNode attributeList = generateStaticMxInternalAttributeList();
        int kind = Tokens.VAR_TOKEN;

        String className = mxmlDocument.getClassName();
        QualifiedIdentifierNode qualifiedIdentifier =
            generateStaticMxInternalQualifiedIdentifier("_" + className + "_StylesInit_done", true);
        MemberExpressionNode memberExpression =
	...
	{
        return addDescriptorInitializerFragments(nodeFactory, model,
