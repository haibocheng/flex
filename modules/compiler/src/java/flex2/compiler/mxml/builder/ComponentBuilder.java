...
public class ComponentBuilder extends AbstractBuilder
{
    ...
    protected class ComponentAttributeHandler extends AttributeHandler
    {
        ...
        {
            // TODO: Other attributes already done in processSpecialAttributes(),
            // but they should be handled here.
                    ...
                    && !namespace.equals(document.getLanguageNamespace()))
            {
                ...
                // If the type implements mx.core.IID and has an id property we 
                // can populate the id property as if it were a normal attribute.
                if (type.isAssignableTo(standardDefs.INTERFACE_IID))
                {
                    property = type.getProperty(localPart);
                }
                // From Flex 4 we'll try to set an id property if a suitable
                // one exists (so long as the attribute was not qualified with
                // a language namespace prefix).
                else if (document.getVersion() >= 4)
                {
                    property = type.getProperty(localPart);
                }
        ...
        {
            if ((document.getVersion() >= 4) && TextParser.isScopedName(localPart))
            {
