 ...
 */
/**
 * This class processes the root node of an MXML document, not just an &lt;Application/&gt;.
 *
 * @author Clement Wong
 */
public class DocumentBuilder extends ComponentBuilder implements MXMLNamespaces
{
	...
	public void analyze(Node node)
	{
		...
		if (unit.isRoot())
		{
			...
			for (Iterator iter = document.getBindingExpressions().iterator(); iter.hasNext(); )
			{
				...
				document.addImport(bexpr.getDestinationTypeName(false), bexpr.getXmlLineNumber());
			}
		}
	}
	...
	protected class NestedDeclarationNodeHandler extends ValueNodeHandler
	{
        ...
        {
            String type = (String) node.getAttribute(StandardDefs.PROP_TYPE).getValue();
            Type elementType = typeTable.getType(NameFormatter.toColon(type));
