...
/*
 * This class is a helper used to generate the code that implements a stateful model, if 
 * state-specific nodes or attributes were declared within a component/document instance.
 * 
 * AdobePatentID="B770"
 */
public class StatesGenerator {
     ...
     * component instance.
     */
    public StatementListNode getStatesASTInitializers(StatesModel model, Context context, StatementListNode statementList)
    {
        StatementListNode results = statementList;
        NodeFactory nodeFactory = context.getNodeFactory();     
        results = genSharedFactoriesAST(model, nodeFactory, statementList);
        results = genStatesAST(model, nodeFactory, results);
        results = genBindingInitializersAST(nodeFactory, results);
        results = genStateInitializersAST(model, nodeFactory, results);
     ...
     */
    private StatementListNode genStatesAST(StatesModel model, NodeFactory nodeFactory, StatementListNode statementList)
    {
        StatementListNode result = statementList;
        ...
        if (!states.isEmpty())
        {
            ...
            for (Iterator<String> iter = states.iterator(); iter.hasNext();  )
            {
                ...
                if (state != null)
                {
                    MemberExpressionNode stateExpression = state.generateDefinitionBody(nodeFactory, bindingsQueue);
     ...
     * initializers for states.
     */
    private StatementListNode genStateInitializersAST(StatesModel model, NodeFactory nodeFactory, StatementListNode statementList)
    {
        StatementListNode result = statementList;
        Set<String> states = model.info.getStateNames();
        int count = 0;
        for (Iterator<String> iter = states.iterator(); iter.hasNext();  )
        {
            ...
            if (state != null)
            {
                ...
                for (Iterator<Initializer> initializers = state.getEvents(); initializers.hasNext(); )
                {
                    ...
                    macromedia.asc.parser.Node valueNode = ei.generateValueExpr(nodeFactory);
