...
public final class StatesModel {
    ...
    {     
        ...
        // If our current node is an array element, take note of the property
        // which our array parent is associated with and ensure declaration of
        // the new node.
        ...
        // Ensure declaration of the new node if this happens to be special case of Array.
        if (model instanceof Array)
            model.ensureDeclaration();
         
        // Ensure that our destination context and model are accessible at runtime by name.
        if (context != null) context.ensureDeclaration();
    ...
    {             
        ...
        // If our current node is an array element, take note of the property
        // which our array parent is associated with.
        String parentProperty = null;
        if (context.getType() == document.getTypeTable().arrayType)
        {            
            parentProperty = (context.getParentIndex() != null) ? context.getParentIndex() : context.getId();
    ...
    private boolean validateReparentType(Model targetModel, Model destination, String parentProperty, ReparentInfo reparentNode)
    {
        ...
        else if (parentProperty != null || parentType.getDefaultProperty() != null)
        {
            ...
            if (property != null && property.getType() == document.getTypeTable().arrayType)
            {
    ...
    {
        if (model.getType() == document.getTypeTable().arrayType || model instanceof MovieClip)
        {
			...
			if (!sibling.isStateSpecific() || intersects)
			{
				...
				if (sibling.getIdIsAutogenerated()  && !(sibling instanceof flex2.compiler.mxml.rep.Reparent))
				{
					document.addDeclaration(sibling.getId(), sibling.getType().getName(), 
                            sibling.getXmlLineNumber(), true, true, true);
				}
			}
    ...
    public void registerStateSpecificProperty(Model model, String property, ValueInitializer value, String stateName)
    {
        ...
        for (Iterator<String> iter = stateNames.iterator(); iter.hasNext(); )
        {
            ...
            if (state != null)
            {
                SetPropertyOverride override = new SetPropertyOverride(model, property, value);
    ...
    public void registerStateSpecificStyle(Model model, String property, ValueInitializer value, String stateName)
    {
        ...
        for (Iterator<String> iter = stateNames.iterator(); iter.hasNext(); )
        {
            ...
            if (state != null)
            {
                SetStyleOverride override = new SetStyleOverride(model, property, value);
                postProcessBindingInstance(value, override);
                state.addOverride(override, model != null ? model.getXmlLineNumber() : 0);
            }
        }
            ...
            ArrayList<Model> relativeNodes, Collection<String> statesList)
    {
        ...
        for (Iterator<Object> iter = states.iterator(); iter.hasNext(); )
        {
            ...
            if (state != null)
            {
                AddItemsOverride override = new AddItemsOverride(parent, parentIndex, parentId, 
                        model.getDefinitionName(), position, relativeNodes, isStyle, isArray, model);
                state.addOverride(override, model != null ? model.getXmlLineNumber() : 0);
            }
        }
    }
    ...
    public class State
    {
         ...
         */
        public MemberExpressionNode generateDefinitionBody(NodeFactory nodeFactory, List<StatesModel.Override> bindingsQueue)
        {
            ...
            for (Iterator<Initializer> iter = model.getPropertyInitializerIterator(false); iter.hasNext(); )
            {
                ...
                if (init instanceof NamedInitializer)
                {
                    ...
                    if (!initName.equals("name") && !initName.equals("stateGroups") && !initName.equals("overrides"))
                    {
                        ...
                        literalString = nodeFactory.literalString(namedInit.getValueExpr());
                        literalField = nodeFactory.literalField(propIdentifier, literalString);
                        stateArguments = nodeFactory.argumentList(stateArguments, literalField);
                    }
                }
            }
            ...
            {
                StatesModel.Override override = iter.next();
                MemberExpressionNode memberExpression = override.generateDefinitionBody(nodeFactory);
    ...
    public class AddItemsOverride extends Override
    {
        ...
        public AddItemsOverride(Model parent, String propertyName, String context, String factory, String position, 
                                ArrayList<Model> relativeNodes, Boolean isStyle, Boolean isArray, Model model )
        {
            this.parent = parent;
        ...
        public void getDefinitionBody(CodeFragmentList list, String indent)
        {
            ...
            if (model.getParent() != null && (model.getParent().getType() == document.getTypeTable().arrayType))
    ...
    public class SetPropertyOverride extends Override
    {
        ...
        public SetPropertyOverride(Model context, String property, ValueInitializer value)
        {
            this.value = value;
        ...
        public MemberExpressionNode generateDefinitionBody(NodeFactory nodeFactory)
        {
            ...
            if (!clear)
            {
                ...
                {
                    valueNode = value.generateValueExpr(nodeFactory);
                }
    ...
    {        
        public SetStyleOverride(Model context, String property, ValueInitializer value)
        {
            super(context, property, value);
        }
    ...
    public class SetEventOverride extends Override
    {
        ...
        public void getDefinitionBody(CodeFragmentList list, String indent)
        {
            ...
            list.add(indent, "name: \"" + event +"\"" + (!clear ? "," : ""), 0);
