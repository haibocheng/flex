...
public abstract class ValueInitializer implements Initializer, Cloneable
{
		...
		if (standardDefs.isIDeferredInstance(lvalueType))
		{
			...
			if (rvalueIsClassRef())
			{
				typeIdentifier = AbstractSyntaxTreeUtil.generateQualifiedIdentifier(nodeFactory,
                                                                                    ...
                                                                                    false);
				args = nodeFactory.argumentList(null, generateInlineRValue(nodeFactory));
			}
			else
			{
				typeIdentifier = AbstractSyntaxTreeUtil.generateQualifiedIdentifier(nodeFactory,
	...
	private Node generateInlineRValue(NodeFactory nodeFactory)
	{
		if (value instanceof Model)
		{
			...
			{
				return asVectorLiteral(nodeFactory, (Vector) value);
			}
			else if (value instanceof Array)
			{
				return asArrayLiteral(nodeFactory, (Array) value);
			}
			else if (value instanceof XML)
			{
				XML xml = (XML)value;
			...
			{
				return asObjectLiteral(nodeFactory, (Model)value);
			}
			else
			{
				assert false : "can't generate inline expr for values of type " + value.getClass();
		...
		{
			return formatExpr(nodeFactory, getLValueType(), value);
		}
	}
    ...
    public StatementListNode generateDefinitionBody(Context context, StatementListNode statementList)
    {
        ...
        if (!modelHasInlineRValue())
        {
            ...
            {
                Vector vector = (Vector) value;
                addAssignExprs(nodeFactory, functionStatementList,
        ...
        if (standardDefs.isIUIComponent(selfType))
        {
            ...
            if (!standardDefs.isRepeater(selfType))
            {
                if (standardDefs.isContainer(selfType))
                {
                    if (movieClip.hasChildren())
                    {
                        functionStatementList = generateDocumentDescriptorAssignment(nodeFactory, movieClip,
                                                                                     varName, functionStatementList);
                ...
                else
                {
                    ...
                    while (childIter.hasNext())
                    {
                        ...
                        if ( !((Model)init.getValue()).isStateSpecific())
                        {
                            ...
                            functionStatementList =
                                init.generateAssignExpr(nodeFactory, functionStatementList, memberExpression);
                        }
                    }
                }
            }
            else
            {
                ...
                VariableDefinitionNode childDescriptorVariableDefinition =
                    generateChildDescriptorVariable(nodeFactory, varName, movieClip);
	...
	public VariableDefinitionNode generateChildDescriptorVariable(NodeFactory nodeFactory, String varName,
																  MovieClip movieClip)
	{
		//list.add("\tvar cd:Array = ", varName, ".childDescriptors = [", line);
		...
		for (Iterator childIter = movieClip.children().iterator(); childIter.hasNext(); )
		{
			...
			MemberExpressionNode memberExpression =
				ImplementationGenerator.addDescriptorInitializerFragments(nodeFactory, movieClip, null, true);
                                                 ...
                                                 StatementListNode statementList, boolean generateDocComments)
	{
		...
		if (hasDefinition())
		{
			...
			if (!(value instanceof Model) || !((Model) value).isDescribed())
			{
				result = generateDefinitionBody(context, result);
			}
		}
                                                                   ...
                                                                   StatementListNode statementList)
    {
        ...
        MemberExpressionNode descriptorsMemberExpression =
            ImplementationGenerator.addDescriptorInitializerFragments(nodeFactory, movieClip,
	 ...
	 *
	 */
	private static void addAssignExprs(NodeFactory nodeFactory,
									   StatementListNode statementList,
									   ...
									   String name)
	{
        while (initIter.hasNext())
        {
            ...
            if (!init.isStateSpecific())
            {
                ...
                statementList =
                    init.generateAssignExpr(nodeFactory, statementList, memberExpression);
            }
        }
	}
	...
	protected Node formatExpr(NodeFactory nodeFactory, Type targetType, Object value)
	{
		...
		else if (targetType.equals(typeTable.functionType))
		{
            ...
            if (valueString.indexOf(DOT) != lastDotIndex)
            {
                ...
                int position = AbstractSyntaxTreeUtil.lineNumberToPosition(nodeFactory, getLineRef());
                MemberExpressionNode base =
                    AbstractSyntaxTreeUtil.generateMemberExpression(nodeFactory, fullyQualifiedClassName, position);
	...
	private static LiteralObjectNode asObjectLiteral(NodeFactory nodeFactory, Model model)
	{
		...
		for (Iterator<Initializer> iter = model.getPropertyInitializerIterator(); iter.hasNext(); )
		{
			...
			LiteralFieldNode literalField =
				nodeFactory.literalField(identifier, init.generateValueExpr(nodeFactory));
