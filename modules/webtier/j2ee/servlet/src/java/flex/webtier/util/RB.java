...
public class RB {
    ...
    public String getString(String key, Object[] array) throws MissingResourceException {
        ...
        // We've got a message, now perform variable substitution. Create properties for
        // each of the values with a key corresponding to the argument number
        Properties p = null;
        if (array != null) {
            p = new Properties();
        ...
        msg = PropertiesUtil.expandDynamicVariables(msg, new Date(), p);
        return msg;
    }
    ...
    // Produce a localized string based on the fields of the translatable object
    // Use the class name (without the package) as the key
    public static String getObjectString(Object instance)
            throws MissingResourceException {
        return RB.getObjectString(ObjectUtil.getClassName(instance), instance);
    }

    // Produce a localized string based on the fields of the translatable object
    public static String getObjectString(String key, Object instance)
            throws MissingResourceException {
        String message = getMessage(instance, BASE_NAME, null, key, null);

        // perform substitution of {foo} with object.getFoo()
        return ObjectUtil.replaceVars(instance, message);
    }

    // Produce a localized string based on the fields of the translatable object
    // Use the class name (without the package) as the key
    public static String getString(Translatable instance)
            throws MissingResourceException {
        return RB.getString(ObjectUtil.getClassName(instance), instance);
    }

    // Produce a localized string based on the fields of the translatable object
    public static String getString(String key, Translatable instance)
            throws MissingResourceException {
        String message = getMessage(instance, BASE_NAME, null, key, null);
